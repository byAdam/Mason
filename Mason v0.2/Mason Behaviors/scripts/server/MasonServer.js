...
var tools = {"interact":[],"destroy":[],"hold":[],"place":[],"use":[],"attack":[]}
...
system.initialize = function() {		
	...
	this.listenForEvent("minecraft:player_attacked_entity", (event) => this.attackEntity(event));
...
system.playerJoin = function(event) {
	if(!primaryClient)
	{
		...
		loggerEvent = this.createEventData('minecraft:script_logger_config')
		loggerEvent.data.log_errors = false
		loggerEvent.data.log_warnings = false
		loggerEvent.data.log_information = false
...
system.getRay = function(player) {
	...
	rotY = rotY.toFixed(8)*1
	rotZ = rotZ.toFixed(8)*1
			...
			for(r=0;r<tool.returns.length;r++)
			{
				switch(tool.returns[r])
				{
					case "playerName":
						...
						break
					case "ray":
						toolEvent.data.ray = this.getRay(player)
				}
			}
...
system.attackEntity = function(event)
{
	player = event.data.player
	attackedEntity = event.data.attacked_entity

	for(t=0;t<tools["attack"].length;t++)
	{
		tool = tools["attack"][t]
		if(attackedEntity.__identifier__==tool.entity)
		{
			toolEvent = this.createEventData(tool.eventName)
			toolEvent.data = {}
			toolEvent.data.eventName = tool.eventName
			toolEvent.data.attackedEntity = attackedEntity
			toolEvent.data.player = player
